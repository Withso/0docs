...
const SeoFloatingPanel = ({
  ...
  const debouncedMetaSave = useDebouncedCallback((value: string) => {
    fetch(`/api/pages/${page.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ metaDescription: value }) }).catch(() => {});
  }, 800);

  const debouncedSlugSave = useDebouncedCallback((value: string) => {
    fetch(`/api/pages/${page.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug: value }) }).catch(() => {});
  }, 800);
