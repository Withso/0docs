          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1.5 text-[12px] text-muted-foreground mb-5">
              ...
              <span className="text-foreground font-medium">
                {latest ? relativeTime(latest.publishedAt) : project?.updatedAt ? relativeTime(project.updatedAt) : "never"}
              </span>
              {(latest?.publisherName || firstName) && (
                <>
                  <span>by</span>
                  <span className="inline-flex items-center gap-1.5">
                    <span className="h-4 w-4 rounded-full bg-accent flex items-center justify-center text-[9px] font-medium text-accent-foreground">
                      {(latest?.publisherName || firstName)[0].toUpperCase()}
                    </span>
                    <span className="text-foreground">{latest?.publisherName || firstName}</span>
                  </span>
                </>
              )}
            </div>
        ...
        <section>
          ...
          <div className="rounded-xl border border-border/60 overflow-hidden">
            ...
            {!loadingActivity && activity.map((row, i) => {
              ...
              const isFirst = i === 0;
              const totalChanges = row.pagesCount + row.sectionsCount + row.blocksCount;
              const initial = (row.publisherName || "M")[0].toUpperCase();
              return (
                <div key={row.id} className={`border-b border-border/60 last:border-b-0 ${expanded ? "bg-muted/10" : ""}`}>
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-[13px] font-medium text-foreground truncate">
                          {row.publisherName || "Manual Update"}
                        </p>
                        <p className="text-[11px] text-muted-foreground">{relativeTime(row.publishedAt)}</p>
                      </div>
                    </div>

                    <div>
                      {row.isActive || isFirst ? (
                    ...
                    <div className="min-w-0">
                      <p className="text-[13px] text-foreground truncate">
                        {row.notes || `Version ${row.versionNumber}`}
                      </p>
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        {totalChanges} {totalChanges === 1 ? "item" : "items"} synced
                        {" · "}{row.pagesCount} pages · {row.blocksCount} blocks
                      </p>
                    </div>
                  ...
                  {expanded && (
                    <div className="px-5 pb-5 pt-1 grid grid-cols-3 gap-4 text-[12px]">
                      <Stat label="Pages" value={row.pagesCount} />
                      <Stat label="Sections" value={row.sectionsCount} />
                      <Stat label="Blocks" value={row.blocksCount} />
                    </div>
                  )}
                </div>
              );
