...
The repo includes `railway.json` + `railway.toml` templates plus a
single-service `Dockerfile`. Railway provisions Postgres for you,
exposes `$PORT`, and points `DATABASE_URL` at the database automatically.

1. Create a new Railway project.
2. Add the **PostgreSQL** plugin.
3. Add a service from this GitHub repo. Railway detects `railway.json`
   and the `Dockerfile`.
4. In the **app service's** Variables tab (not the Postgres service),
   optionally set:
   - `ADMIN_EMAIL` + `ADMIN_PASSWORD` — bootstrap admin user.
     Otherwise the first person to sign up becomes the admin.
   - `OPENAI_API_KEY` — enables the "Ask docs" feature.
   - `SMTP_URL` + `SMTP_FROM` — enables real password-reset and invite
     emails. Without these, reset/invite links are printed to the
     server logs.
   - `STORAGE_BACKEND=s3` + `S3_*` — store media in object storage
     instead of Postgres BLOBs.

   You don't need to set `DATABASE_URL` (Railway injects it from the
   Postgres plugin) or `SESSION_SECRET` (the server auto-generates and
   persists one on first boot).
5. Deploy. Migrations run on boot; the healthcheck at `/api/healthz`
   confirms the service is up.

By default, uploaded media is stored as Postgres BLOBs (zero infra
config). Set `STORAGE_BACKEND=s3` to offload to S3-compatible storage
when your media corpus grows.
...
This builds a single app image (API + bundled frontend) and runs it
alongside Postgres. Open `http://localhost:8081`. Put a reverse proxy
in front for HTTPS — see [`SELFHOSTING.md`](./SELFHOSTING.md).
...
├── docker-compose.yml  Postgres + single app service.
├── Dockerfile          Single-service: API + bundled frontend.
├── railway.json        Railway deploy template.
...
| `SESSION_SECRET` | Optional override. If unset, the server auto-generates a secret on first boot and stores it in `system_settings`. |
...
| `STORAGE_BACKEND` | `postgres` (default, BLOBs in DB) or `s3` (S3-compatible object storage). |
| `S3_*` | Bucket / region / credentials when `STORAGE_BACKEND=s3`. See `.env.example` for the full list. |
| `MAX_UPLOAD_BYTES` | Per-upload limit. Default 20 MB. |
...
| `CORS_ALLOWLIST` | Comma-separated origins to allow in production. Only needed when the API and web app live on different hosts. |
